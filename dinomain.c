  ...
  if(spacing == 4){
    for(int i = 0; i<4; i++){
      *(line1) = "   "; // start by creating the first 3 spaces
      srand(randSeed); // seed random each time with a new value to make sure we get random numbers every time.
      obstPosition = rand()%2; // obst position 1 = up, 0 = down;
			if(obstPosition == 0){
				strcat(*(line1), "x");
				strcat(*(line2), " ");
			} else {
	...
	if(spacing == 16){
      ...
      srand(randSeed);
      obstPosition = rand()%2;
			*(line1) = "               "; // start by creating the first 15 spaces
			if(obstPosition == 0){
				strcat(*(line1), "x");
				strcat(*(line2), " ");
			} else {
